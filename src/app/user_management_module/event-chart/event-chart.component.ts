...
// @ViewChild('canvas')
  ...
  getDate() {
    console.log(this.eventChartForm.value.from_date);
    this.startdat=this.eventChartForm.get('from_date')?.value ? this.eventChartForm.value.from_date.toLocaleDateString("en-CA") : this.olddateraw;
    this.enddat=this.eventChartForm.get('startdate')?.value ? this.eventChartForm.value.to_date.toLocaleDateString("en-CA") : this.currentDate.toLocaleDateString("en-CA");

    this.startdate = this.eventChartForm.get('from_date')?.value ? this.convertDateToEpochTimeStamp(this.eventChartForm.get('from_date')?.value)  : this.olddate;

    this.enddate = this.eventChartForm.get('startdate')?.value
      ? this.convertDateToEpochTimeStamp(this.eventChartForm.get('startdate')?.value)
      : this.convertDateToEpochTimeStamp(this.currentDate);
      console.log(this.eventChartForm.value.user)
      console.log(this.eventChartForm.get('user')?.value)
      this.get_user = this.eventChartForm.get('user')?.value =='All' ? 'All'  : (this.eventChartForm.get('user')?.value ? (this.eventChartForm.get('user')?.value.USER_ID):'All');
  ...
  getEvent(){
    // this.graphService.fetchEvent('eventChart', this.userId)
    // .subscribe(respArray => {
    //   this.loginConfig = respArray;

    //   this.eventGraph(loginCount, loginDate)
    // })
    this.myevent=true;
    this.getDate();
    this.eventChartForm.patchValue({
      from_date: this.startdate,
      to_date: this.enddate,
      user: this.get_user,
    });
    let data: [] = this.eventChartForm.value;

    this._apiSubscription2=this.graphService.fetchEvent('eventChart',data, this.userId)
    .subscribe(respArray => {
        this.loginConfig = respArray;
