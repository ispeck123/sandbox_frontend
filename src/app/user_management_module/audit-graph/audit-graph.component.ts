...
@Component({
  selector: 'app-audit-graph',
  templateUrl: './audit-graph.component.html',
  styleUrls: ['./audit-graph.component.css']
})
export class AuditGraphComponent implements OnInit {
  // @ViewChild('canvas')
  ...
  getDate() {
    console.log(this.loginChartForm.value.from_date);
    this.startdat=this.loginChartForm.get('from_date')?.value ? this.loginChartForm.value.from_date.toLocaleDateString("en-CA") : this.olddateraw;
    this.enddat=this.loginChartForm.get('startdate')?.value ? this.loginChartForm.value.to_date.toLocaleDateString("en-CA") : this.currentDate.toLocaleDateString("en-CA");

    this.startdate = this.loginChartForm.get('from_date')?.value ? this.convertDateToEpochTimeStamp(this.loginChartForm.get('from_date')?.value)  : this.olddate;

    this.enddate = this.loginChartForm.get('startdate')?.value
      ? this.convertDateToEpochTimeStamp(this.loginChartForm.get('startdate')?.value)
      : this.convertDateToEpochTimeStamp(this.currentDate);
      console.log(this.loginChartForm.value.user)
      console.log(this.loginChartForm.get('user')?.value)
    this.get_user = this.loginChartForm.get('user')?.value =='All' ? 'All'  : (this.loginChartForm.get('user')?.value ? (this.loginChartForm.get('user')?.value.USER_ID):'All');
  ...
  getLogin(){
    this.myVar=true;
    this.getDate();
    this.loginChartForm.patchValue({
      from_date: this.startdate,
      to_date: this.enddate,
      user: this.get_user,
    });
    let data: [] = this.loginChartForm.value;

    this._apiSubscription2=this.graphService.fetchLogin('loginChart',data, this.userId)
    .subscribe(respArray => {
  ...
  loginGraph( loginData:any){


   /* this.chart = new Chart('login',{
      type: 'bar',
       data : {
        labels: loginDate
        ,
